# Third-party Credits

FreeSCP bundles or links against:

## Qt 6.8.3
- License: LGPL-3.0
- Project: [https://code.qt.io](https://code.qt.io)
- Notes: Dynamically linked; no local modifications.
- License file: [LICENSES/Qt-LGPL-3.0.txt](LICENSES/Qt-LGPL-3.0.txt)

## libssh2
- License: BSD-3-Clause
- Project: [https://www.libssh2.org](https://www.libssh2.org)
- License file: [LICENSES/libssh2-BSD-3-Clause.txt](LICENSES/libssh2-BSD-3-Clause.txt)

## libcurl
- License: curl
- Project: [https://curl.se/libcurl/](https://curl.se/libcurl/)
- License file: [LICENSES/libcurl-curl-license.txt](LICENSES/libcurl-curl-license.txt)

## tinyxml2
- License: zlib
- Project: [https://github.com/leethomason/tinyxml2](https://github.com/leethomason/tinyxml2)
- License file: [LICENSES/tinyxml2-zlib-license.txt](LICENSES/tinyxml2-zlib-license.txt)

## OpenSSL 3
- License: Apache-2.0
- Project: [https://www.openssl.org](https://www.openssl.org)
- License file: [LICENSES/OpenSSL-Apache-2.0.txt](LICENSES/OpenSSL-Apache-2.0.txt)

## zlib
- License: zlib
- Project: [https://zlib.net](https://zlib.net)
- License file: [LICENSES/zlib-license.txt](LICENSES/zlib-license.txt)
